...
def process_ctd_data(expocode, eventlog_fname, root_dir):
    ...
    # process ctd data
    _, cast_flist = bgc_io.load_ctd_casts(df_event_log, expocode, root_dir=root_dir)
...
def process_niskin_data(
    ...
    # create bottle file
    if new_btl_file:
        btl_fname = bgc_io.create_bottle_file(df_event_log, expocode, root_dir=root_dir)
    else:
    ...
    if extract_ctdsal:
        bgc_io.extract_niskin_salts(
            df_event_log, btl_fname, niskin_length, root_dir=root_dir
        )
