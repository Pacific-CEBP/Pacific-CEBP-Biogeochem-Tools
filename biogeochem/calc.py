...
def _sea_pressure(ds, Psurf=101325.0):
...
def _absolute_salinity(ds):
    ds["SA"] = xr.apply_ufunc(gsw.SA_from_SP, ds["SP"], ds["P"], ds["lon"], ds["lat"])
    ds["SA"].attrs = {
    ...
    for cast in ds_btl["cast_number"].values:
        if ds_btl.sel(cast_number=cast)["cast_f"] == 2:
            ...
            if missing_niskin_time:
                ...
                ds_cast_nisk = ds_cast.sel(
                    P=slice(Pnisk - 0.75 * niskin_length, Pnisk + 0.75 * niskin_length)
                )
            ...
            if not missing_niskin_time:
                ...
                Pmax = np.ceil((ds_cast["P"].max() / 5.0)) * 5.0
                axT.set_ylim([0.0, Pmax.values])
                ...
                plot_fname = "{0:s}_{1:03d}.pdf".format(
                    ds_cast.expocode, ds_cast.cast_number
                )
...
def recalc_alkalinity(
    btl_fname, dic_fname, root_dir=None, btl_dir=None, dic_dir=None, titration_dir=None
):
    if root_dir is not None:
        btl_dir = os.path.join(root_dir, "btl")
