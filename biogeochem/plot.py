...
def plot_casts(cast_flist, root_dir=None, cast_dir=None, plot_dir=None):
    ...
    for cast_fname in cast_flist:
        ...
        fig, (axT, axC, axS) = plt.subplots(
            nrows=1, ncols=3, figsize=(6, 8), sharey=True, constrained_layout=True
        )
        ...
        axS.set_title(
            "UTC {0:s}\n{1:s}".format(date_str, time_str), loc="right", fontsize=9
        )
...
def plot_ctdsal_qc(btl_fname, root_dir=None, btl_dir=None, plot_dir=None):
    ...
    ax.plot([0.0, 35.0], [0.0, 35.0], "k-", linewidth=0.5)
    ...
    for i, lbl in enumerate(ds_btl_ok["cast_number"].values):
        ax.annotate(
            lbl,
            (ds_btl_ok["salinity"][i], ds_btl_ok["ctdsal"][i]),
            size=10,
            weight="bold",
            color="r",
        )
