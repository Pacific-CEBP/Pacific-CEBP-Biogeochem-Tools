...
def write_nutrients_for_cindy(btl_fname, root_dir=None, btl_dir=None):
    if root_dir is not None:
        btl_dir = os.path.join(root_dir, "btl")

    print("Exporting nutrient data for Cindy Wright...", end="", flush=True)
    ...
    df_btl = ds_btl.to_dataframe()
    df_btl.to_excel(
        os.path.join(btl_dir, (btl_fname + ".xlsx")),
        columns=[
            "expocode",
            #"cast_number",
            "station_id",
